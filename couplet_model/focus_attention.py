...
from enc_dec_iter import get_rhyme, get_pos
...
class FocusSeq2Seq(object):
    ...
    def couplet_predict(self, enc_string, word2idx, word2pos, word2rhyme, arg_params, 
        knowledge ='view', pos = 'vocab', kind='rhyme', pad = 0, eos = 1, unk = 2):
        # ------------------------- bind data to symbol ---------------------------
        idx2word = {}
        for key in word2idx:
        ...
        if pos == 'vocab':
            ...
            pos_data = get_pos(enc_string)
            rhyme_data = get_rhyme(enc_string)
    ...
    def couplet_knowledge_predict(self, enc_string, word2idx, word2pos, word2rhyme, arg_params, 
        knowledge ='view', pos = 'vocab', kind='pos', pad = 0, eos = 1, unk = 2):
        # ------------------------- bind data to symbol ---------------------------
        idx2word = {}
        for key in word2idx:
            idx2word[word2idx[key]] = key
